...
<configuration>
    ...
    <#else>
    <@property "tdt.jdbc.url" "jdbc:hive2://${inceptor_url}/tdt;authentication=kerberos;kuser=tdt/${localhostname?lower_case}@${service.realm};keytab=${service.keytab};krb5conf=/etc/krb5.conf;principal=hive/${inceptor_server}@${service.realm}"/>
    </#if>
...
<#if service.auth="kerberos">
    ...
    <@property "tdt.server.authentication.kerberos.principal" "tdt/${localhostname?lower_case}@${service.realm}"/>
    <#else>
