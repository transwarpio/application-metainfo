...
export BATCH_SIZE=${(service['logstash.memory.ratio']?number * service['logstash.container.limits.memory']?number)?floor * 1.25 * 1024}
export LS_HEAP_SIZE=${(service['logstash.memory.ratio']?number * service['logstash.container.limits.memory']?number)?floor}g
