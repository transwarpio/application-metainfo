...
<#if edition == "HEAVY">
  ...
  dashboard-source:
    predefined:
      type: TDH_DASHBOARD
      platformClassPath:
       - dashboard/tos2.0/tdh/init-dashboard.yml
<#if edition == "HEAVY">
       - dashboard/tos2.0/tdh/init-deluxe-log-dashboard.yml
<#elseif edition == "LIGHT">
       - dashboard/tos2.0/tdh/init-lite-log-dashboard.yml
</#if>
...
<#else>
  ...
  thread-pool:
    ...
    queue-capacity: 100
  filters:
    platform: "level,service_instance_name"
    tenant: "level,service_instance_name"
  index:
...
alert:
  ...
  rule-source:
    predefined:
      type: TDH_RULE
      platformClassPath: alert/tos2.0/tdh/init-rule.yml
