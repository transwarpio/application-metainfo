filter {
  ...
  grok {
    patterns_dir => ["/etc/${service.sid}/conf/logstash/patterns"]
...
### General log process ###
  grok {
    ...
    match => {
      "message" => [
        ...
        "^%{KUNDB_LOGLEVEL:level}%{KUNDB_TIMESTAMP:datetime}\s+%{GREEDYDATA:message}$"
      ]
    }
    overwrite => ["message"]
    remove_tag => ["_grokparsefailure"]
  ...
  if [service] =~ "manager_operation" {
    grok {
      patterns_dir => ["/etc/${service.sid}/conf/logstash/patterns"]
      break_on_match => true
      match => { "message" => [
        "%{MANAGER_OPERATION_AUDIT}"
      ]}
    }
  }
  ...
  if [service] =~ "kundb" and [is_audit]  {
    json {
      source => "message"
    }
    if [Error] {
      mutate {
        add_field => {"SQLExecResult" => "Fail"}
      }
    } else {
      mutate {
        add_field => {"SQLExecResult" => "Success"}
      }
    }
    mutate {
      add_field => { "datetime" => "%{[time]}" }
    }
  } else if [service] =~ "kundb" {
      if [level] == "I" {
        mutate {
          update => { "level" => "INFO" }
        }
