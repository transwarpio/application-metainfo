...
<#if service['profiler.container.limits.memory'] != "-1" && service['profiler.memory.ratio'] != "-1">
  <#assign limitsMemory = service['profiler.container.limits.memory']?number
    memoryRatio = service['profiler.memory.ratio']?number
    profilerMemory = limitsMemory * memoryRatio * 1024>
<#else>
  <#if service[.data_model["localhostname"]]['inceptor.profiler.memory']??>
    <#assign profilerMemory=service[.data_model["localhostname"]]['inceptor.profiler.memory']?trim?number>
  <#else>
    <#assign profilerMemory=8192>
  </#if>
</#if>
...
export PROFILER_PORT=${(dependencies.INCEPTOR['inceptor.profiler.ui.port'])!9093}
export COMPACTOR_PORT=${(dependencies.INCEPTOR['inceptor.compactor.ui.port'])!9918}
<#elseif dependencies.NEWSEARCH_STORAGE??>
export METASTORE_PORT=${dependencies.NEWSEARCH_STORAGE['hive.metastore.port']}
export PROFILER_PORT=${dependencies.NEWSEARCH_STORAGE['inceptor.profiler.ui.port']}
export COMPACTOR_PORT=${dependencies.NEWSEARCH_STORAGE['inceptor.compactor.ui.port']}
<#else>
...
<#elseif dependencies.NEWSEARCH_STORAGE??>
  <#list dependencies.NEWSEARCH_STORAGE.roles["INCEPTOR_METASTORE"] as role>
    <#assign metastoreUris += [role.hostname]>
  </#list>
<#else>
...
export KEYTAB=/etc/${service.sid}/conf/argodb_computing.keytab
...
export EXECUTOR_KEYTAB=/etc/${service.sid}/conf/argodb_computing.keytab
export MASTERPRINCIPAL=hive/${localhostname}
export KEYTAB=/etc/${service.sid}/conf/argodb_computing.keytab
export KRB_PLUGIN_ENABLE=true
<#else>
...
# NewSearch WebUI Switch
export SPARK_UI_ISARGODB=true
