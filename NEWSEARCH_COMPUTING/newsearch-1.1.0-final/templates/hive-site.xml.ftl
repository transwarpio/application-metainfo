...
<#if dependencies.NEWSEARCH_STORAGE??>
    <#assign storage=dependencies.NEWSEARCH_STORAGE master_group=[]>
    ...
    <@property "ngmr.search.shiva.mastergroup" master_group?join(",")/>
</#if>
...
<#elseif dependencies.NEWSEARCH_STORAGE??>
    <#list dependencies.NEWSEARCH_STORAGE.roles["INCEPTOR_METASTORE"] as role>
        <#assign uris += [("thrift://" + role.hostname + ":" + dependencies.NEWSEARCH_STORAGE["hive.metastore.port"])]>
    </#list>
    <@property "hive.metastore.service.id" "${dependencies.NEWSEARCH_STORAGE.sid}"/>
<#else>
