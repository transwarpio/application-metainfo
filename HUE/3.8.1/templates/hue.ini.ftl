...
[desktop]
  ...
  secret_key=${service.sid}

  # Webserver listens on this address and port
  http_host=0.0.0.0
  http_port=${service['hue.desktop.http.port']}
...
<#if service['auth']=='ldap'>
  ldap_username=hue
  ldap_password=${service['hue.user.password']}
</#if>
...
<#if service['auth']=='ldap'>
    base_dn=${service['hue.ladp.base.dn']}
</#if>

    # URL of the LDAP server
    ## ldap_url=ldap://auth.mycompany.com
<#if service['auth']=='ldap'>
    ldap_url=${service['hue.ladp.url']}
</#if>
...
<#if service['auth']=='ldap'>
    bind_dn=${service['hue.ladp.bind.dn']}
</#if>
    ...
    ## bind_password=
<#if service['auth']=='ldap'>
    bind_password=${service['hue.ladp.bind.password']}
</#if>
...
<#if service['auth']=='ldap'>
      user_name_attr=${service['hue.ladp.user.name.attribute']}
</#if>
  ...
  [[database]]
    engine=sqlite3
    name=/var/lib/${service.sid}/desktop.db
...
[hadoop]
  ...
  [[hdfs_clusters]]
    ...
    [[[default]]]
      ...
      fs_defaultfs=hdfs://${dependencies.HDFS.nameservices[0]}/

      # NameNode logical name.
      logical_name=hdfs://${dependencies.HDFS.nameservices[0]}/

      # Use WebHdfs/HttpFs as the communication mechanism.
      # Domain should be the NameNode or HttpFs host.
      # Default port is 14000 for HttpFs.
      webhdfs_url=http://${dependencies.HDFS.roles.HDFS_HTTPFS[0]['hostname']}:14000/webhdfs/v1/
...
<#if dependencies.YARN??>
      resourcemanager_host=${dependencies.YARN.roles.YARN_RESOURCEMANAGER[0]['hostname']}
</#if>
...
<#if dependencies.YARN.roles.YARN_RESOURCEMANAGER?size gt 1>
      logical_name=rm1
<#else>
      logical_name=${dependencies.YARN.roles.YARN_RESOURCEMANAGER[0]['hostname']}
</#if>
...
<#if dependencies.YARN?? && dependencies.YARN.auth?? && dependencies.YARN.auth=="kerberos">
      ...
      resourcemanager_api_url=http://${dependencies.YARN.roles.YARN_RESOURCEMANAGER[0]['hostname']}:${dependencies.YARN['resourcemanager.webapp.port']}
...
[liboozie]
  ...
  oozie_url=http://${dependencies.OOZIE.roles.OOZIE_SERVER[0]['hostname']}:${dependencies.OOZIE['oozie.http.port']}/oozie
...
<#if dependencies.INCEPTOR??>
    hive_server_host=${dependencies.INCEPTOR.roles.INCEPTOR_SERVER[0]['hostname']}
</#if>
  # Port where HiveServer2 Thrift server runs on.
  ## hive_server_port=10000
<#if dependencies.INCEPTOR??>
  hive_server_port=${dependencies.INCEPTOR['hive.server2.thrift.port']}
</#if>

  # Hive configuration directory, where hive-site.xml is located
  ## hive_conf_dir=/etc/hive/conf
<#if dependencies.INCEPTOR??>
  hive_conf_dir=/etc/${dependencies.INCEPTOR.sid}/conf
</#if>
...
<#if dependencies.HYPERBASE?? && dependencies.HYPERBASE.roles.HYPERBASE_THRIFT??>
  <#if dependencies.HYPERBASE.roles.HYPERBASE_THRIFT?size gt 0>
    <#assign hbase_clusters="">
    <#list dependencies.HYPERBASE.roles.HYPERBASE_THRIFT as thrift>
      <#assign hbase_clusters="${hbase_clusters}(Cluster_${thrift['hostname']}|${thrift['hostname']}:9090),">
    </#list>
    hbase_clusters=${hbase_clusters?remove_ending(",")}
  </#if>
</#if>
...
<#if dependencies.HYPERBASE??>
    hbase_conf_dir=/etc/${dependencies.HYPERBASE.sid}/conf
</#if>
